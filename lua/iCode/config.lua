...
_ICODECONFIG = _ICODECONFIG or {}

config.values = _ICODECONFIG
...
config.set = function(opts)
  ...
  if next(opts) ~= nil then
    for k, v in pairs(opts) do
      local typ = check_type(_ICODECONFIG[k], v, k)
      if typ ~= "table" then
        _ICODECONFIG[k] = v
      else
        consume_opts(k, _ICODECONFIG[k], v)
      end
    end
  else
    if vim.tbl_isempty(_ICODECONFIG) then
      _ICODECONFIG = defaults
      config.values = _ICODECONFIG
    end
  end
end
